...
select u.id as user_id,
       ...
       -- DEUX casquettes internes possibles, et il faut les DEUX. Trois
       -- personnes portent dans Bubble « Admin » ET « Recruiter Core Team ».
       -- La version d'origine prenait `limit 1` sur les trois rôles mêlés :
       -- elle n'en gardait qu'un, et l'index unique sur collaborateur_id
       -- interdisait la seconde ligne. Ces trois-là perdaient une casquette.
       -- Depuis la séparation des portails (migration 20260910290000), elles
       -- peuvent avoir les deux — encore faut-il les leur donner.
       exists (select 1 from public.user_role r
                where r.user_id = u.id and r.role = 'Admin')                         as est_admin_src,
       (select case r.role when 'Recruiter Support Crew' then 'support'
                           else 'recruteur' end
          from public.user_role r
         where r.user_id = u.id
           and r.role in ('Recruiter Core Team','Recruiter Support Crew')
         -- Core Team l'emporte sur Support Crew si les deux sont posés.
         order by case r.role when 'Recruiter Core Team' then 0 else 1 end
         limit 1)                                                                    as role_recruteur_src
from public."user" u;
...
select distinct on (coalesce(k.fiche_talent_id::text, k.collaborateur_id::text, k.user_id))
       k.user_id, k.collaborateur_id, k.fiche_talent_id, k.contact_client_id,
       k.est_admin_src, k.role_recruteur_src
from t_casquette k
...
-- Un accès par casquette, et le PORTAIL EST DÉCLARÉ. Il se calculait tant
-- qu'il était une colonne générée ; il ne l'est plus depuis que le
-- back-office s'est séparé du recruteur. L'omettre écrirait NULL — et le
-- NOT NULL posé par 20260910310000 fera échouer franchement plutôt que de
-- laisser passer un accès muet.
--
-- QUATRE insertions désormais, pas trois : les deux portails internes se
-- peuplent séparément, et un même collaborateur peut recevoir les deux.

-- Le back-office : les Admin de Bubble.
insert into app.acces (compte_id, collaborateur_id, portail, role_interne, actif, cree_le, maj_le)
select reprise.uid('compte#'||k.user_id), k.collaborateur_id,
       'backoffice'::app.portail, 'admin'::app.role_interne, true, now(), now()
from t_compte k
join app.compte c on c.id = reprise.uid('compte#'||k.user_id)
where k.collaborateur_id is not null and k.est_admin_src
on conflict do nothing;

-- Le recrutement : Core Team, Support Crew — et le collaborateur SANS aucun
-- rôle Bubble, qui reçoit recruteur par défaut. Ce défaut est conservé tel
-- quel : sans lui, son compte n'aurait aucun accès et `compte_a_un_acces`
-- le refuserait au commit. Il concernait 4 personnes à la reprise du 08/09.
insert into app.acces (compte_id, collaborateur_id, portail, role_interne, actif, cree_le, maj_le)
select reprise.uid('compte#'||k.user_id), k.collaborateur_id,
       'recruteur'::app.portail,
       coalesce(k.role_recruteur_src, 'recruteur')::app.role_interne, true, now(), now()
from t_compte k
join app.compte c on c.id = reprise.uid('compte#'||k.user_id)
where k.collaborateur_id is not null
  and (k.role_recruteur_src is not null or not k.est_admin_src)
on conflict do nothing;

insert into app.acces (compte_id, fiche_talent_id, portail, actif, cree_le, maj_le)
select reprise.uid('compte#'||k.user_id), k.fiche_talent_id,
       'talent'::app.portail, true, now(), now()
from t_compte k
join app.compte c on c.id = reprise.uid('compte#'||k.user_id)
where k.fiche_talent_id is not null
on conflict do nothing;

insert into app.acces (compte_id, contact_client_id, portail, actif, cree_le, maj_le)
select reprise.uid('compte#'||k.user_id), k.contact_client_id,
       'entreprise'::app.portail, true, now(), now()
from t_compte k
join app.compte c on c.id = reprise.uid('compte#'||k.user_id)
