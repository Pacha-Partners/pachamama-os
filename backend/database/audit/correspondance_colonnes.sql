-- =====================================================================
-- AUDIT DE NON-PERTE — NIVEAU VALEUR
--
-- Le rapprochement par LIGNES ne prouve rien : une ligne peut être
-- insérée avec toutes ses colonnes vides. Ce qui se mesure ici, ce sont
-- les VALEURS — pour chaque colonne du miroir, combien étaient
-- renseignées, et combien le sont dans sa destination.
--
-- Trois natures :
--   reprise  → la colonne a une destination, les comptes doivent coïncider
--   fusion   → plusieurs sources vers une destination, le compte cible
--              doit être au moins égal à la plus grande des sources
--   abandon  → décision documentée, la valeur ne migre pas
-- =====================================================================

drop table if exists audit_map;
